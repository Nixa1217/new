...
export default function LifestyleGallery() {
  ...
  // setSelectedImage state is removed as it's no longer needed due to DialogTrigger handling.
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const currentImages = visionBoard?.lifestyle_images || [];
      updateVisionBoard.mutate({
        ...visionBoard,
        lifestyle_images: [...currentImages, file_url]
      });
    } catch (error) {
  ...
  const handleDeleteImage = (imageUrl) => {
    ...
    updateVisionBoard.mutate({
      ...visionBoard,
      lifestyle_images: currentImages.filter(url => url !== imageUrl)
    });
  };
  ...
  const handleDragEnd = (result) => {
    ...
    const currentImages = Array.from(visionBoard?.lifestyle_images || []);
    ...
    updateVisionBoard.mutate({
      ...visionBoard,
      lifestyle_images: currentImages
    });
  };

  const images = visionBoard?.lifestyle_images || [];
    ...
    <div className="min-h-screen bg-[#F5EFE7] p-3">
      <div className="max-w-6xl mx-auto pt-8 space-y-6 pb-32">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-3xl font-bold text-[#5C4A3A]">Lifestyle Vision</h1>
          <label htmlFor="lifestyle-upload">
            <Button
              disabled={uploading}
        ...
        <DragDropContext onDragEnd={handleDragEnd}>
          <Droppable droppableId="lifestyle-images" direction="vertical">
            {(provided) => (
              <div
                ...
                <AnimatePresence>
                  {images.map((imageUrl, index) => (
                    <Draggable key={imageUrl} draggableId={imageUrl} index={index}>
                      {(draggableProvided) => (
                        <motion.div
                          key={imageUrl} // Key for AnimatePresence to track motion.div
