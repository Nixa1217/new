...
export default function PartnerGallery() {
  ...
  // const [selectedImage, setSelectedImage] = useState(null); // Removed: no longer used
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const currentImages = visionBoard?.partner_images || [];
      updateVisionBoard.mutate({
        ...visionBoard,
        partner_images: [...currentImages, file_url]
      });
    } catch (error) {
  ...
  const handleDeleteImage = (imageUrl) => {
    ...
    updateVisionBoard.mutate({
      ...visionBoard,
      partner_images: currentImages.filter(url => url !== imageUrl)
    });
  };
  ...
  const handleDragEnd = (result) => {
    ...
    const currentImages = Array.from(visionBoard?.partner_images || []);
    ...
    updateVisionBoard.mutate({
      ...visionBoard,
      partner_images: currentImages
    });
  };

  const images = visionBoard?.partner_images || [];
    ...
    <div className="min-h-screen bg-[#F5EFE7] p-3">
      <div className="max-w-6xl mx-auto pt-8 space-y-6 pb-32">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-3xl font-bold text-[#5C4A3A]">Partner Vision</h1>
        ...
        <DragDropContext onDragEnd={handleDragEnd}>
          <Droppable droppableId="partner-images" direction="vertical">
            {(provided) => (
              <div
                ...
                <AnimatePresence>
                  {images.map((imageUrl, index) => (
                    ...
                    <Draggable key={imageUrl} draggableId={imageUrl} index={index}>
                      {(draggableProvided) => (
                        <motion.div // motion.div now wraps the draggable element
                          ...
                          <Dialog>
                            <DialogTrigger asChild>
                              <img
                                ...
                                // Removed onClick={() => setSelectedImage(imageUrl)} as it's no longer used
                              />
                            </DialogTrigger>
