...
export default function FitnessGallery() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const currentImages = visionBoard?.fitness_images || [];
      updateVisionBoard.mutate({
        ...visionBoard,
        fitness_images: [...currentImages, file_url]
      });
    } catch (error) {
  ...
  const handleDeleteImage = (imageUrl) => {
    ...
    updateVisionBoard.mutate({
      ...visionBoard,
      fitness_images: currentImages.filter(url => url !== imageUrl)
    });
  };
  ...
  const handleDragEnd = (result) => {
    ...
    const currentImages = Array.from(visionBoard?.fitness_images || []);
    ...
    updateVisionBoard.mutate({
      ...visionBoard,
      fitness_images: currentImages
    });
  };

  const images = visionBoard?.fitness_images || [];
    ...
    <div className="min-h-screen bg-[#F5EFE7] p-3">
      <div className="max-w-6xl mx-auto pt-8 space-y-6 pb-32">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-3xl font-bold text-[#5C4A3A]">Fitness Vision</h1>
          <label htmlFor="fitness-upload">
            <Button
              disabled={uploading}
        ...
        <DragDropContext onDragEnd={handleDragEnd}>
          <Droppable droppableId="fitness-images" direction="vertical">
            {(provided) => (
              <div
