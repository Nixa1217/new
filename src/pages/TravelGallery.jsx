...
export default function TravelGallery() {
  ...
  // selectedImage state is no longer used for displaying full size images,
  // as the Dialog component handles that directly.
  ...
  const updateVisionBoard = useMutation({
    ...
    onSuccess: () => {
      ...
      // setUploading(false) is handled here for upload success
      // it doesn't interfere with drag-and-drop as uploading state
      // is only set to true during file upload.
      setUploading(false);
    }
  });
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const currentImages = visionBoard?.travel_images || [];
      updateVisionBoard.mutate({
        ...visionBoard,
        travel_images: [...currentImages, file_url]
      });
      // The onSuccess of updateVisionBoard will handle setting uploading to false
    } catch (error) {
  ...
  const handleDeleteImage = (imageUrl) => {
    ...
    updateVisionBoard.mutate({
      ...visionBoard,
      travel_images: currentImages.filter(url => url !== imageUrl)
    });
  };
  ...
  const handleDragEnd = (result) => {
    ...
    const currentImages = Array.from(visionBoard?.travel_images || []);
    ...
    updateVisionBoard.mutate({
      ...visionBoard,
      travel_images: currentImages
    });
  };

  const images = visionBoard?.travel_images || [];
    ...
    <div className="min-h-screen bg-[#F5EFE7] p-3">
      <div className="max-w-6xl mx-auto pt-8 space-y-6 pb-32">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-3xl font-bold text-[#5C4A3A]">Travel Vision</h1>
        ...
        <DragDropContext onDragEnd={handleDragEnd}>
          <Droppable droppableId="travel-images" direction="vertical">
            {(provided) => (
              <div
                        ...
                        <Dialog>
                          <DialogTrigger asChild>
                            <img
                              ...
                              // setSelectedImage(imageUrl) is removed as it's not used
                            />
                          </DialogTrigger>
